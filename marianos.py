...
def process_product(product, store_dict): 
    ...
    image_url = next((i['url'] for size in ['xlarge', 'large', 'medium', 'small'] 
                      for i in product['item']['images'] 
                      if i['perspective'] == 'front' and i['size'] == size), 
                     next((i['url'] for i in product['item']['images'] if i['perspective'] == 'top' and i['size'] == 'xlarge'), 
                          product['item']['images'][0]['url'] if product['item']['images'] else ""))
    ...
    if promo_description and not re.match(pattern, promo_description):
        ...
        final =  promo_validator.process(product_data)
        return final
...
async def main():
    ...
    output_filename = f"marianos_{datetime.now().date()}.csv"
